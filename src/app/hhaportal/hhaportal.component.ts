...
export class HhaportalComponent implements OnInit {
  ...
  constructor(
    ...
    private _locationsrv: LocationService,
    ...
    azureMap.events.add("ready", function () {
        ...
        .then(function () {
          ...
          datasource.add(
            new atlas.data.Feature(
              new atlas.data.Point([Number(longitude), Number(latitude)]),
              { name: userName }
            )
          );
      ...
      for (let i = 0; i < ClientList.length; i++) {
        let clientName =
          ...
          " " +
          ClientList[i].middleName +
          " " +
          ClientList[i].lastName;
        // var point = new atlas.Shape(
        //   new atlas.data.Point([Number(ClientList[i].longitude), Number(ClientList[i].latitude)])
        //   ,{name:clientName}
        //   );
        // points.push(point);
      ...
      // azureMap.events.add('mouseleave', symbolLayer, function (){
      //   popup.close();
      // });
    });
  }
