...
export class EmpAttendanceComponent implements OnInit {
  ...
  model = new Attendance(0, 0, 0, '', '', '');
  currentUser: UserModel;
  ClientList = Array<ItemsList>();
  attendanceObjList: any;
  AbsenceActivityList: ItemsList[] = [];
  _startDate: Date;
  _endDate: Date;
  ...
  constructor(
    ...
    this.comApi.getMaster(MasterType.AbsenceActivity).subscribe((response) => {
      if (response.result) {
        this.AbsenceActivityList = response.data;
      }
    });
  ...
  saveAttendance() {
    ...
    this.model.reason = Number(this.model.reason);
