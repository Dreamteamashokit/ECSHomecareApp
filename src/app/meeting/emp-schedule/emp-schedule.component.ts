...
export class EmpScheduleComponent implements OnInit {
  ...
  onChange(e: any): void {
    ...
    if (e.target.checked) {
      ...
      this._toDate = tDate;
      this.setInitailTime(this.model.meetingDate);
  ...
  setInitailTime(_from: string) {
    ...
    from.setMilliseconds(today.getMilliseconds());

    if (today.getFullYear() === from.getFullYear() &&
      today.getDate() === from.getDate() &&
      today.getMonth() === from.getMonth()) {
      from.setMinutes(from.getMinutes() + 15);
      this._startMinTime = from;    
    }
    this._startTime = from;
    let endTime = new Date(from);
  ...
  public fromChanged(arg: any): void {

    let _from = new Date(arg);
    this.setInitailTime(this.datepipe.transform(_from, 'yyyy-MM-dd') || "");
  }
