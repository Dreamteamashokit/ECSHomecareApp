...
import {
  AvailbilityReponse,
  ClientGeoProvisions,
} from "src/app/models/availbility/availbility-response";
...
export class SearchAvailbilityComponent implements OnInit {
  ...
  constructor(
    ...
    this.comSrv.getEmpTypeList().subscribe((response) => {
      this.empTypeList = response.data.filter(
        (x) => x.itemId == 2 || x.itemId == 5
      );
    });
  ...
  caseChange(e: any): void {
    if (e.target.value != "0: undefined") {
      this.comSrv.getUsersGeoProvision(Number(e.target.value)).subscribe({
        next: (response) => {
          if (response.result) {
            ...
            this.provisionsTypeList = this.provisionsTypeList.map((elem) => {
              elem.IsChecked =
                response.data.provisions.indexOf(elem.itemId) != -1
                  ? true
                  : false;
              return elem;
            });
        ...
        error: (err) => {
          ...
          this.toastr.infoToastr(
            "Some technical issue exist, Please contact to admin !",
            "Info!"
          );
  ...
  search() {
    ...
    this.searchModel.provisionsList = this.provisionsTypeList
      .filter((x) => x.IsChecked == true)
      .map((y) => Number(y.itemId));
    this.searchModel.fromDate =
      this.datepipe.transform(this._fromDate, "dd-MM-yyyy") || "";
    this.searchModel.toDate =
      this.datepipe.transform(this._toDate, "dd-MM-yyyy") || "";
    this.searchModel.timeIn =
      this.datepipe.transform(this._startTime, "h:mm a") || "";
    this.searchModel.timeOut =
      this.datepipe.transform(this._endTime, "h:mm a") || "";
    ...
    this.locSrv.searchAvailbility(reqObj).subscribe({
      next: (response) => {
        if (response.result) {
          // debugger;
          ...
          this.toastr.infoToastr(
            "no record found for current criteria !",
            "Info!"
          );
  ...
  findRadius(item: ClientSearch) {
    this.availbilityList.forEach(
      (loc: AvailbilityReponse) => {
        this.locSrv
          .getDistance(
            item.latitude,
            item.longitude,
          ...
          .subscribe({
            ...
            complete: () => {
              var sortedArray: AvailbilityReponse[] = this.availbilityList.sort(
                (a: AvailbilityReponse, b: AvailbilityReponse) => {
                  debugger;
  ...
  BindTerm() {
    ...
    if (addend === 5) {
      ...
      this.termList.push(
        new ItemsList(
          current.getDate(),
          this.datepipe.transform(new Date(current), "dd MMM YYYY") || ""
        )
      );
    }
    if (addend > 5) {
