...
export class PatientComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (objUser != null && objUser != undefined) {
      ...
      this.userName =
        objUser.firstName + " " + objUser.middleName + " " + objUser.lastName;
  ...
  GetClockinDetailsByUserId(userId: number) {
    this._employeeService
      .GetClockinDetailsByUserId(userId)
      .subscribe((response) => {
        if (response.result) {
          this.clockinDetails = response.data;
          this.clockInTime = this.clockinDetails.clockInTime;
          this.IsLoad = false;
        } else {
          this.IsLoad = false;
          this.toastr.infoToastr(
            "HHA/Employee does not have any clients!",
            "Info!"
          );
  ...
  HHAClockout(model: NgForm) {
    ...
    setTimeout(() => {
      if (
        this.clockinDetails != null &&
        this.clockinDetails != undefined &&
        (this.clockinDetails.clockOutTime == null ||
          this.clockinDetails.clockOutTime == undefined)
      ) {
        if (
          model.value.BedBath &&
          model.value.SpongeBath &&
          model.value.Footcare &&
          model.value.Skincare
        ) {
          ...
          if (!this.signaturePad.isEmpty() && !this.HHAsignaturePad.isEmpty()) {
            if (
              ...
              this.HHAsignaturePad.toDataURL() != undefined
            ) {
              this.model.HHAUserSignature = this.HHAsignaturePad.toDataURL();
            }

            if (
              this.signaturePad.toDataURL() != null &&
              this.signaturePad.toDataURL() != undefined
            ) {
              this.model.ClientSignature = this.signaturePad.toDataURL();
            }

            this._accountService
              .HHAClockout(this.model)
              .subscribe((response) => {
