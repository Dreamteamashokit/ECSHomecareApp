...
import { UploadFileFolder ,DocumentView} from 'src/app/models/employee/upload-file-folder';
...
export class DocumentComponent implements OnInit {
  ...
  DeleteFolder(folderId: number) {
    ...
    if (isOk) {
      this.IsLoad = true;
      this.docSrv.deleteFolder(folderId, this.fModel.userId).subscribe(response => {
        if (response.result) {
          this.IsLoad = false;
          this.GetFolderList(this.fModel.userId);
        }
